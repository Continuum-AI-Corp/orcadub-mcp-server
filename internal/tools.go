...
func RegisterTools(s *mcp.Server, c *Client) {
	...
	mcp.AddTool(s, &mcp.Tool{Name: "dub_get", Description: "Get a dub job's status/progress (status: queued | in_progress | completed | failed). On completed the response carries an output_url (short-lived presigned mp4 link); on failed an error message."}, t.dubGet)
